...
router.post('/login',[
    ...
    let success=false
    if (!errors.isEmpty()) {
      return res.status(400).json({succcess,errors: errors.array()})
    }
    ...
    try{
        ...
        if(!user){
            return res.status(400).json({succcess,error:"Please login with correct credintials"});
        }
...
//ROUTE3: Authenticate user at /api/auth/user
